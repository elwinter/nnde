...
The analytical form of the equation is:
  ...
  G([x, y, z, t], Y, delY, del2Y) = dY_dt - D*(d2Y_dx2 + d2Y_dy2 + d2Y_dz2)
  = 0
...
del2Y is the vector (d2Y/dx2, d2Y_dy2, d2Y/z2, d2Y/dt2)
...
Ya(x, y, z, t) = exp(-3*pi**2*D*t)*sin(pi*x)*sin(pi*y)*sin(pi*z)
...
def del2A(xyzt):
    ...
    (x, y, z, t) = xyzt
    d2A_dx2 = -pi**2*(1 - t)*sin(pi*x)*sin(pi*y)*sin(pi*z)
    d2A_dy2 = -pi**2*(1 - t)*sin(pi*x)*sin(pi*y)*sin(pi*z)
    d2A_dz2 = -pi**2*(1 - t)*sin(pi*x)*sin(pi*y)*sin(pi*z)
    d2A_dt2 = 0
...
delYa = [dYa_dx, dYa_dy, dYa_dz, dYa_dt]
...
del2Ya = [d2Ya_dx2, d2Ya_dy2, d2Ya_dz2, d2Ya_dt2]
