"""Tests for the diff1d_halfsine module."""
...
from nnde.differentialequation.examples.diff1d_halfsine import (
    D,
    G, f0, f1, Y0, bc,
...
class TestBuilder(unittest.TestCase):
    ...
    def test_dYa_dx(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                dYa_dx_ref = pi*exp(-pi**2*D*t)*cos(pi*x)
    ...
    def test_dYa_dt(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                dYa_dt_ref = -pi**2*D*exp(-pi**2*D*t)*sin(pi*x)
    ...
    def test_d2Ya_dx2(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                d2Ya_dx2_ref = -pi**2*exp(-pi**2*D*t)*sin(pi*x)
    ...
    def test_d2Ya_dt2(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                d2Ya_dt2_ref = (-pi**2*D)**2*exp(-pi**2*D*t)*sin(pi*x)
