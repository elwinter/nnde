"""Tests for the diff1d_halfsine_increase module."""
...
from nnde.differentialequation.examples.diff1d_halfsine_increase import (
    ...
    dG_dY,
    dG_ddY_dx, dG_ddY_dt, dG_ddelY,
    dG_dd2Y_dx2, dG_dd2Y_dt2, dG_ddel2Y,
    A, delA, del2A,
...
class TestBuilder(unittest.TestCase):
    ...
    def test_dG_ddY_dx(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                self.assertAlmostEqual(dG_ddY_dx(xt, Y, delY, del2Y),
                                       dG_ddY_dx_ref)
    ...
    def test_dG_ddY_dt(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                self.assertAlmostEqual(dG_ddY_dt(xt, Y, delY, del2Y),
                                       dG_ddY_dt_ref)
    ...
    def test_dG_d2Y_dx2(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                self.assertAlmostEqual(dG_dd2Y_dx2(xt, Y, delY, del2Y),
                                       dG_d2Y_dx2_ref)
    ...
    def test_dG_d2Y_dt2(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                self.assertAlmostEqual(dG_dd2Y_dt2(xt, Y, delY, del2Y),
                                       dG_d2Y_dt2_ref)
    ...
    def test_delA(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                delA_ref[0] = a*t + (1 - t)*pi*cos(pi*x)
                delA_ref[1] = a*x - sin(pi*x)
                for i in range(2):
    ...
    def test_Ya(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                Ya_ref = a*t*x + sin(pi*x)*(cosh(pi**2*t*D) - sinh(pi**2*t*D))
    ...
    def test_dYa_dx(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                dYa_dx_ref = (
                    a*t + pi*cos(pi*x)*(cosh(pi**2*t*D) - sinh(pi**2*t*D))
                )
                for k in range(1, kmax + 1):
                    dYa_dx_ref += (
    ...
    def test_dYa_dt(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                dYa_dt_ref = (
                    a*x + sin(pi*x)*pi**2*D*(sinh(pi**2*t*D) - cosh(pi**2*t*D))
                )
                for k in range(1, kmax + 1):
                    dYa_dt_ref += (
                        2*(-1)**k*a*exp(-k**2*pi**2*t*D)*D*sin(k*pi*x)/(k*pi)
                    )
    ...
    def test_delYa(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                self.assertAlmostEqual(delYa[0](xt), dYa_dx(xt))
                self.assertAlmostEqual(delYa[1](xt), dYa_dt(xt))
    ...
    def test_d2Ya_dx2(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                d2Ya_dx2_ref = (
                    -pi**2*sin(pi*x)*(cosh(pi**2*t*D) - sinh(pi**2*t*D))
                )
                for k in range(1, kmax + 1):
                    d2Ya_dx2_ref += (
                        -2*(-1)**k*a*(1 - exp(-pi**2*t*D*k**2))*sin(pi*x*k)
    ...
    def test_d2Ya_dt2(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                d2Ya_dt2_ref = (
                    sin(pi*x)*pi**4*D**2*(cosh(pi**2*t*D) - sinh(pi**2*t*D))
                )
                for k in range(1, kmax + 1):
                    d2Ya_dt2_ref += (
                        -2*(-1)**k*a*exp(-pi**2*t*D*k**2)*k*pi*D**2*sin(pi*x*k)
                    )
    ...
    def test_del2Ya(self):
        ...
        for x in xx:
            for t in tt:
                xt = (x, t)
                self.assertAlmostEqual(del2Ya[0](xt), d2Ya_dx2(xt))
                self.assertAlmostEqual(del2Ya[1](xt), d2Ya_dt2(xt))
