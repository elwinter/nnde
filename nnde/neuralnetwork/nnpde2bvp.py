"""
NNPDE2BVP - Class to solve 2nd-order PDE BVP.

This module provides the functionality to solve 2-variable PDE boundary
value problems problems using a neural network.
...
from nnde.trialfunction.pde2bvptrialfunction import PDE2BVPTrialFunction
...
class NNPDE2BVP(SLFFNN):
    ...
    def __init__(self, eq, nhid=DEFAULT_NHID):
        ...
        if m == 2:
            self.tf = PDE2BVPTrialFunction(eq.bc, eq.delbc, eq.del2bc)
        else:
            raise NNDEException
...
#         return s
    ...
    def _train_delta(self, x, opts=DEFAULT_OPTS):
        ...
        # # <HACK>
        # w[...] = [[-0.03283963, -0.04337479, -0.0333208],
        #           [ 0.08916026,  0.01477627,  0.03570686]]
        # u[...] =  [-0.01133743,  0.08387997, -0.06781313]
        # v[...] =  [0.0123421,    0.08048297, -0.04861869]
        ...
        #            -0.02365936, -0.03380184, -0.03492679,  0.03708778, -0.03626765]]
        ...
        for epoch in range(maxepochs):
            ...
            # print("d2Yt_dwdx.shape =", d2Yt_dwdx.shape)
            ...
            # print("Yt = %s" % Yt)
            # print("dY_dx = %s" % delYt[:, 0])
            # print("dY_dy = %s" % delYt[:, 1])
            # print("d2Y_dx2 = %s" % del2Yt[:, 0])
            # print("d2Y_dy2 = %s" % del2Yt[:, 1])
